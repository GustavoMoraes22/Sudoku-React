...
const checkValid = (grid, row, col, num) => {
    if (checkRow(grid, row, num) && checkCol(grid, col, num) && checkBox(grid, col, row, num)) return true
...
const checkBox = (grid, col, row, num) => {
...
export const compareSudokus = (currentSudoku, solverSudoku) => {
    ...
    for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
            if (currentSudoku[i][j] !== solverSudoku[i][j]) {
